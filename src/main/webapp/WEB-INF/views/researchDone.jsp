        ...
        <div class="tbl_box">
          <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl_type01" summary="설문조사">
           ...
           <%-- <td class="tl">
				  ...
				  <c:forEach items="${surveyVO}" var="item"> --%>
					
		               	<c:choose>
		                	<c:when test="${'Y' eq surveyVO.finish_yn}">
		                		<td>
		                			<a href="#" onclick="searchPopup(${surveyVO.sur_seq}); return false;">
		                				<img src="/resources/images/sub/btn/btn_view.gif" alt="결과보기"/>
		               				</a>
		              				</td>
		                	</c:when>
               	...
               	<!-- varStatus="status"를 통해서 index를 얻어올 수 있음 -->
               	   ...
               	   <div class="research">
                       ...
                       <input type="hidden" name="answer[]" value="">
                        ...
                        <ul>
	                        <li>
	                        	<input type="radio" id="radio1${item.suri_seq}" class="radioButton" name="radio${item.suri_seq}" value="1" onchange="onchangeRadio(this);" disabled>
					  			<label for="radio1${item.suri_seq}">① ${item.question1}</label>
	                        </li>
	                        <li>
	                        	<input type="radio" id="radio2${item.suri_seq}" class="radioButton" name="radio${item.suri_seq}" value="2" onchange="onchangeRadio(this);" disabled>
					  			<label for="radio2${item.suri_seq}">② ${item.question2}</label>
	                        </li>
	                        <li>
	                        	<input type="radio" id="radio3${item.suri_seq}" class="radioButton" name="radio${item.suri_seq}" value="3" onchange="onchangeRadio(this);" disabled>
					 			 <label for="radio3${item.suri_seq}">③ ${item.question3}</label>
	                        </li>
	                        <li>
	                         	<input type="radio" id="radio4${item.suri_seq}" class="radioButton" name="radio${item.suri_seq}" value="4" onchange="onchangeRadio(this);" disabled>
					  			<label for="radio4${item.suri_seq}">④ ${item.question4}</label>
	                        </li>
	                        <li>
	                        	<input type="radio" id="radio5${item.suri_seq}" class="radioButton" name="radio${item.suri_seq}" value="5" onchange="onchangeRadio(this);" disabled>
					 			<label for="radio5${item.suri_seq}">⑤ ${item.question5}</label>
	                        </li>
	                        <li>상세 사유: <input type="text" id="test01" name="choice_reason[]" placeholder="사유를 입력하세요. (선택)" value="" class="inp" style="width:200px;" disabled /> </li>
                        </ul>
					</div>
